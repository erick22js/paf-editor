		...
		cornerloop: for(var c=0; c<corners.length; c++){
			...
			if(box.width <= corner.width && 
				...
				for(var k=0; k<corners.length; k++){
					...
					/*
					if((korner.left < box.x+box.width) && (korner.left+korner.width > box.x)){
						if((box.y < korner.top+korner.height) && (box.y+box.height > korner.top)){
							korner.height = box.y-korner.top;
						}
					}
					else if((korner.top < box.y+box.height) && (korner.top+korner.height > box.y)){
						if((box.x < korner.left+korner.width) && (box.x+box.width > korner.left)){
							korner.width = box.x-korner.left;
						}
					}
					/*
					if((korner.left < corner.left) && (korner.top <= (box.y+box.height))){
						if((korner.left+korner.width) > corner.left){
							korner.width = corner.left-korner.left;
							var rheight = korner.height - ((box.y+box.height) - korner.top);
							if(rheight <= korner.height){
								corners.push({
									"left": korner.left, "top": box.y+box.height,
									"width": korner.width,
									"height": rheight
								});
							}
					...
					if((korner.top < corner.top) && (korner.left < (box.x+box.width))){
						if((korner.top+korner.height) > corner.height){
							korner.height = corner.top-korner.top;
							var rwidth = korner.width - ((box.x+box.width) - korner.left);
							if(rwidth <= korner.width){
								corners.push({
									"left": box.x+box.width, "top": korner.top,
									"width": rwidth,
									"height": korner.height,
								});
							}
						}
					}
					*/
					/*if(korner.width <= 0 || korner.height <= 0){
						corners.splice(k, 1);
						k--;
					}*/
				}
